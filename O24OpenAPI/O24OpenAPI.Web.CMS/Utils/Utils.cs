...
public static class Utils
{
    ...
    ///
    /// </summary>
    public static ErrorInfoModel AddActionError(
        string type,
        string typeError,
        string info,
        string code,
    ...
    public static async Task<List<UserCommand>> ReadUserCommandCSV(string path, string appCode)
    {
        var listData = new List<UserCommand>();
        ...
        )
        {
            ...
            while (await csv.ReadAsync())
            {
                var record = csv.GetRecord<S_USRCMD>();
                var entity = new UserCommand()
                {
                    ApplicationCode = appCode,
                    CommandId = record.CMDID,
                    ParentId = record.PARENTID,
                    CommandName = record.CMDNAME,
                    CommandNameLanguage = record.CMDVALUE,
                    CommandType = record.CMDTYPE,
                    CommandURI = record.URI,
                    Enabled = record.ISENABLE == 1,
                    IsVisible = record.ISVISIBLE == 1,
                    DisplayOrder = record.CMDIDX,
                    GroupMenuVisible = record.ISVISIBLE.ToString(),
                    GroupMenuId = GetGroupId(record, dicCommandModule),
                };
                listData.Add(entity);
            }
        }
