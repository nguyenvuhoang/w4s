...
public static class JsonExtension
{
    ...
    public static T MapToModel<T>(this JToken jObject)
    {
        ...
        try
        {
            ...
            var result = System.Text.Json.JsonSerializer.Deserialize<T>(strObject);
            return result;
        }
        catch (Exception ex)
        {
    ...
    /// <param name="jObject"></param>
    /// <returns></returns>
    public static T JsonConvertToModel<T>(this JArray jObject)
    {
        try
        {
            string strObject = JsonConvert.SerializeObject(jObject);
            var result = JsonConvert.DeserializeObject<T>(strObject);
            return result;
        }
        catch (Exception ex)
        {
    ...
    )
    {
        ...
        try
        {
            ...
            var result = System.Text.Json.JsonSerializer.Deserialize<T>(jsonSerial, options);
            return result;
        }
        catch (Exception ex)
        {
            O9Utils.ConsoleWriteLine($"Error in JsonSerializerToModel: {ex}");
            throw;
        }
    }
        ...
        where TModel : class
    {
        ...
        var renamedData = dataSearch.RenameFields(propertyNames);

        var resultList = renamedData.Select(item => item.ToObject<TModel>()).ToList();
    ...
    {
        var _context = EngineContext.Current.Resolve<JWebUIObjectContextModel>();

        var dictionary = _context.Bo.GetFoInput().input;
