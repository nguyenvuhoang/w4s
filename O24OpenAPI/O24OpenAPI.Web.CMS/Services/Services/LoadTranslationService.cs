...
public class LoadTranslationService(
...
) : ITranslationService
{
    ...
    public List<TranslationEntry> Load()
    {
        ...
        foreach (var lang in languages.Keys)
        {
            ...
            {
                throw new Exception($"Error loading translation file for language '{lang}': {ex.Message}", ex);
            }
        }

        return keySet.Select(key => new TranslationEntry
        {
            Key = key,
    ...
    private static Dictionary<string, string> FlattenJson(JsonElement jsonElement, string parentKey = "")
    {
    ...
    private static async Task WriteUnflattenedJsonAsync(string path, Dictionary<string, string> flat, JsonSerializerOptions options)
    {
