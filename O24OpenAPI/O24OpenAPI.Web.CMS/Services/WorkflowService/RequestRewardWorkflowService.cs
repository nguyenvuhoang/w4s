...
{
    private readonly IRequestRewardService _requestrewardService =
        EngineContext.Current.Resolve<IRequestRewardService>();
    private readonly IRepository<D_USER_REWARD> _userrewardRepository =
        EngineContext.Current.Resolve<IRepository<D_USER_REWARD>>();
    private readonly IRepository<D_REWARDS> _rewardRepository = EngineContext.Current.Resolve<
        IRepository<D_REWARDS>
    >();
    ...
    public async Task<WorkflowScheme> Insert(WorkflowScheme workflow)
    {
        ...
        return await Invoke<BaseTransactionModel>(
            ...
            async () =>
            {
                ...
                var reward = _rewardRepository
                    .Table.Where(r => r.Id == newRequestReward.GiftID)
                    .FirstOrDefault();
                ...
                if (reward == null)
                {
                    throw new O24OpenAPIException(
                        "InvalidReward",
                        "The specified reward does not exist."
                    );
                }

                var userRewards = await _userrewardRepository
                    .Table.Where(x => x.UserCode == model.CurrentUserCode)
                    .Select(t => t.TotalPoint - t.UsedPoint)
                    .ToListAsync();
                ...
                if (totalPoint < reward.RequiredPoints)
                {
                    throw new O24OpenAPIException(
                        "InsufficientPoints",
                        "Not enough points to request the reward."
                    );
                }
    ...
    public async Task<WorkflowScheme> Search(WorkflowScheme workflow)
    {
        ...
        return await Invoke<SearchRequestRewardModel>(
            workflow,
            async () =>
            {
