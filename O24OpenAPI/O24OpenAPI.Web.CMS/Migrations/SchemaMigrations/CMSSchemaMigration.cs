...
public class CMSSchemaMigration : AutoReversingMigration
{
    ...
    public override void Up()
    {
        ...
        if (!Schema.Table(nameof(LearnApiExecutionLog)).Exists())
        {
            Create.TableFor<LearnApiExecutionLog>();
        }
        ...
        if (!Schema.Table(nameof(MediaFile)).Exists())
        {
            ...
            Create.Index("IDX_MediaFile_TrackerCode")
                .OnTable(nameof(MediaFile))
                .OnColumn(nameof(MediaFile.TrackerCode)).Ascending().WithOptions().NonClustered();

            Create.Index("IDX_MediaFile_Status")
                .OnTable(nameof(MediaFile))
                .OnColumn(nameof(MediaFile.Status)).Ascending().WithOptions().NonClustered();

            Create.Index("IDX_MediaFile_ExpiredOnUtc")
                .OnTable(nameof(MediaFile))
                .OnColumn(nameof(MediaFile.ExpiredOnUtc)).Ascending().WithOptions().NonClustered();

            Create.Index("IDX_MediaFile_FileHash")
                .OnTable(nameof(MediaFile))
                .OnColumn(nameof(MediaFile.FileHash)).Ascending().WithOptions().NonClustered();

            Create.Index("IDX_MediaFile_CreatedOnUtc")
               .OnTable(nameof(MediaFile))
               .OnColumn(nameof(MediaFile.CreatedOnUtc)).Ascending().WithOptions().NonClustered();
            Create.UniqueConstraint("UQ_MediaFile_TrackerCode_FileHash")
              .OnTable(nameof(MediaFile))
        ...
        if (!Schema.Table(nameof(MediaStaging)).Exists())
        {
            ...
            Create.Index("IDX_MediaStaging_FileHash")
                .OnTable(nameof(MediaStaging))
                .OnColumn(nameof(MediaStaging.FileHash)).Ascending().WithOptions().NonClustered();

            Create.Index("IDX_MediaStaging_Status")
                .OnTable(nameof(MediaStaging))
                .OnColumn(nameof(MediaStaging.Status)).Ascending().WithOptions().NonClustered();

            Create.Index("IDX_MediaStaging_ExpiredOnUtc")
                .OnTable(nameof(MediaStaging))
                .OnColumn(nameof(MediaStaging.ExpiredOnUtc)).Ascending().WithOptions().NonClustered();
        }
        ...
        if (!Schema.Table(nameof(FormFieldDefinition)).Exists())
        {
            ...
            Create.Index("UQ_FormFieldDefinition_FormId_FieldName")
            ...
            Create.Index("IDX_FormFieldDefinition_UpdatedOnUtc")
                .OnTable(nameof(FormFieldDefinition))
        ...
        if (!Schema.Table(nameof(AppTypeConfig)).Exists())
        {
            ...
            Create.Index("UQ_AppTypeConfig_AppCode")
                .OnTable(nameof(AppTypeConfig))
                .OnColumn(nameof(AppTypeConfig.AppCode)).Ascending()
                .WithOptions().Unique();

            Create.Index("IDX_AppTypeConfig_UpdatedOnUtc")
                .OnTable(nameof(AppTypeConfig))
                .OnColumn(nameof(AppTypeConfig.UpdatedOnUtc)).Ascending()
                .WithOptions().NonClustered();
        }
        if (!Schema.Table(nameof(TranslationLanguages)).Exists())
        {
            ...
            Create.Index("IDX_TranslationLanguages_ChannelId_Language")
            .OnTable(nameof(TranslationLanguages))
            .OnColumn(nameof(TranslationLanguages.ChannelId)).Ascending()
            .OnColumn(nameof(TranslationLanguages.Language)).Ascending()
            .WithOptions().NonClustered();

            Create.Index("IDX_TranslationLanguages_UpdatedAt")
                .OnTable(nameof(TranslationLanguages))
                .OnColumn(nameof(TranslationLanguages.UpdatedAt)).Descending()
                .WithOptions().NonClustered();
        }
        ...
        if (!Schema.Table(nameof(Form)).Exists())
        {
            ...
            Create.UniqueConstraint("UC_Form")
                  .OnTable(nameof(Form))
                  .Column(nameof(Form.FormId));
        }
        ...
        if (!Schema.Table(nameof(WorkflowDefinition)).Exists())
        {
            ...
            Create.UniqueConstraint("UC_WorkflowDefinition_WFId")
                .OnTable(nameof(WorkflowDefinition))
                .Column(nameof(WorkflowDefinition.WFId));
        }
        ...
        if (!Schema.Table(nameof(WorkflowStep)).Exists())
        {
            ...
            Create.UniqueConstraint("UC_WorkflowStep_WFId_StepCode")
                .OnTable(nameof(WorkflowStep))
                .Columns(nameof(WorkflowStep.WFId), nameof(WorkflowStep.StepCode));
        }
        ...
        if (!Schema.Table(nameof(WorkflowStepLog)).Exists())
        {
            ...
            Create.UniqueConstraint("UC_WorkflowStepLog_ExecutionId")
                .OnTable(nameof(WorkflowStepLog))
                .Columns(nameof(WorkflowStepLog.ExecutionId));
        }

        if (!Schema.Table(nameof(UserSessions)).Exists())
        {
            Create.TableFor<UserSessions>();

            Create.UniqueConstraint("UC_UserSessions_Ssesionid")
                .OnTable(nameof(UserSessions))
                .Columns(nameof(UserSessions.Ssesionid));
        }
