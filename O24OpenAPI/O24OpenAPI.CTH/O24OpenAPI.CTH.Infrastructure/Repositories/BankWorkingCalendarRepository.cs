...
public class BankWorkingCalendarRepository(
    ...
    : EntityRepository<BankWorkingCalendar>(eventPublisher, dataProvider, staticCacheManager),
...
public async Task UpdateWorkingDateTripletAsync(long id, DateTime newWorkingDate, DateTime? previousWorkingDate, DateTime? nextWorkingDate)
{
    newWorkingDate = newWorkingDate.Date;
    await Table
        .Where(x => x.Id == id)
