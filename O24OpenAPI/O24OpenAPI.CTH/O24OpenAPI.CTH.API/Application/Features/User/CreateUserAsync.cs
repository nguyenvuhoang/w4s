...
{
    public class CreateUserAsyncCommand : BaseTransactionModel, ICommand<UserResponseModel>
    {
    ...
    [CqrsHandler]
    public class CreateUserAsyncHandle(
    ...
    ) : ICommandHandler<CreateUserAsyncCommand, UserResponseModel>
    {
        ...
        )
        {
            ...
            try
            {
                ...
                var isUserExisting = await userAccountRepository.IsExist(request.UserName);
                if (isUserExisting)
                {
                ...
                var userchannel = string.IsNullOrEmpty(request.ContractType)
                    ? "BO"
                ...
                // 5. Create UserInRole
                var listUserRole = await userRoleRepository.GetByRoleTypeAsync(request.UserType);
                ...
                var nameParts = new[] { request.FirstName, request.MiddleName, request.LastName };
