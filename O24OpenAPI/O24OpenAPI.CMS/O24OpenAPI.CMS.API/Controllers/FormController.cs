...
public class FormController(
    ILoadFormService formService,
    IJwtTokenService jwtTokenService,
    ICTHGrpcClientService cthGrpcClientService,
    WorkContext workflowContext
) : BaseController
{
    ...
    public async Task<IActionResult> Load([FromBody] FormModelRequest request)
    {
        ...
        try
        {
            ...
            {
                return Unauthorized(
                    new { error = "session_not_found", message = "User session is invalid" }
                );
            }
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(
                ...
                new { error = "cth_unavailable", message = "Can not connect to Control Hub" }
            );
        }
