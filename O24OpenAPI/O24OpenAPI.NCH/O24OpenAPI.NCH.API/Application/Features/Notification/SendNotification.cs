...
) : ICommandHandler<SendNotificationCommand, bool>
{
    ...
    public async Task<bool> SendNotification(NotificationRequestModel model)
    {
        ...
        try
        {
            string type = (model?.NotificationType ?? "").Trim().ToUpperInvariant();
            ...
            switch (type)
            {
                ...
                case "SMS":
                    SMSRequestModel sms = model as SMSRequestModel ?? MapToSms(model);
                    return await SendSMS(sms);

                case "TELE":
                    TelegramSendModel tele = model as TelegramSendModel ?? MapToTelegram(model);
    ...
    public async Task<bool> SendSMS(SMSRequestModel model)
    {
        try
        {
            string message = string.IsNullOrWhiteSpace(model.Message)
            ...
            string transactionId = model.RefId;
    ...
    public async Task<bool> SendEmailAsync(SendMailRequestModel model)
    {
        EmailSendOut emailLog = new()
        {
            ...
            Receiver = !string.IsNullOrWhiteSpace(model.Receiver)
                ...
                : (
                    ...
                    && model.DataTemplate.TryGetValue("email", out object emailVal)
                        ? emailVal?.ToString()
                        : null
                ),
        ...
        try
        {
            MailConfig getMailConfig = await mailConfigRepository.GetByConfigId(model.ConfigId);
            ...
            if (model.AttachmentBase64Strings != null && model.AttachmentBase64Strings.Count > 0)
            {
                ...
                {
                    string base64 = model.AttachmentBase64Strings[i];
                    string filename = model.AttachmentFilenames[i];
                    try
                    {
            ...
            {
                foreach (O24MimeEntity entity in model.MimeEntities)
                {
                    try
                    {
