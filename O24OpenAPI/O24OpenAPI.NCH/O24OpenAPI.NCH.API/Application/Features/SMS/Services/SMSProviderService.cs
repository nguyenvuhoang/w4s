...
public class SMSProviderService(
    ...
    ISMSProviderStatusRepository smsProviderStatusRepository,
    ISMSMappingResponseRepository smsMappingResponseRepository
) : ISMSProviderService
{
    /// <summary>
    /// Defines the _smsProviderConfigRepository
    /// </summary>
    private readonly ISMSProviderConfigRepository _smsProviderConfigRepository =
        smsProviderConfigRepository;

    /// <summary>
    /// Defines the _smsProviderRepository
    /// </summary>
    private readonly ISMSProviderRepository _smsProviderRepository = smsProviderRepository;

    /// <summary>
    /// Defines the _smsSendOutRepository
    /// </summary>
    private readonly ISMSSendOutRepository _smsSendOutRepository = smsSendOutRepository;

    /// <summary>
    /// Defines the _smsProviderStatusRepository
    /// </summary>
    private readonly ISMSProviderStatusRepository _smsProviderStatusRepository =
        smsProviderStatusRepository;

    /// <summary>
    /// Defines the _smsMappingResponseRepository
    /// </summary>
    private readonly ISMSMappingResponseRepository _smsMappingResponseRepository =
    ...
    {
        List<SMSProvider> providers = await _smsProviderRepository.Table.Where(p => p.IsActive).ToListAsync();
    ...
    )
    {
        ...
        SMSProvider providerMain = await GetProviderByPhoneNumber(phoneNumber);
        ...
        SMSProvider provider = await _smsProviderRepository.Table.FirstOrDefaultAsync(p =>
    ...
    {
        List<SMSProviderConfig> configs = await _smsProviderConfigRepository
        ...
        if (!configDict.TryGetValue("SOAP_TEMPLATE", out string template))
        {
        ...
        switch (provider.ProviderName?.ToUpperInvariant())
        {
            ...
            case "LTC":
                values.TryGetValue("RECEIVERPHONE", out string receiverPhone);
                values.TryGetValue("TRANSACTIONID", out string transactionId);
                string dynamicKey = $"{provider.ApiUsername}{transactionId}{receiverPhone}";
            ...
            case "ETL":
                ...
                values.TryGetValue("RECEIVERPHONE", out string eltReceiverPhone);
                values.TryGetValue("TRANSACTIONID", out string etlTransactionId);
                string sign = Etl.GenerateSign(
    ...
    {
        string wsdlUrl = await _smsProviderConfigRepository
            .Table.Where(x =>
        ...
        HttpResponseMessage response = await client.PostAsync(wsdlUrl, content);
    ...
    {
        List<SMSProvider> activeProviders = await _smsProviderRepository
            .Table.Where(p => p.IsActive)
            .ToListAsync();

        foreach (SMSProvider provider in activeProviders)
        {
            ...
            try
            {
                string dummyConfig = await _smsProviderConfigRepository
                    .Table.Where(x =>
                ...
                Dictionary<string, string> dummyData = string.IsNullOrWhiteSpace(dummyConfig)
                    ? []
                ...
                HttpResponseMessage response = await client.PostAsync(provider.ApiUrl, content);
                responseString = await response.Content.ReadAsStringAsync();

                stopwatch.Stop();
                List<SMSMappingResponse> allMappings = await _smsMappingResponseRepository.Table.ToListAsync();
                ...
                (string msgId, bool isSuccess, int resultCode, string resultDesc, string providerKey, string rawCode) =
    ...
    )
    {
        ...
        foreach ((string ProviderName, string PhoneNumber, string Message, string TransactionId, string EndToEnd) msg in messages)
        {
            ...
            var t = Task.Run(
                async () =>
                {
                    try
                    {
                        SMSProvider provider = await _smsProviderRepository.Table.FirstOrDefaultAsync(
                        ...
                        SendSOAPResponseModel resp = await SendSOAPRequestAndLogAsync(
                            provider,
                            phoneNumber,
                            msg.Message,
                            soapXml,
    ...
    )
    {
        ...
        SMSSendOut sendOut = CreateSendOut(
        ...
        try
        {
            ...
            string requestHeader = await AddDynamicSoapHeaders(content, provider.ProviderName);
            sendOut.RequestHeader = requestHeader;
            sendOut.FullRequest = BuildFullHttpRequest(wsdlUrl, "POST", content);

            HttpResponseMessage response = await httpClient.PostAsync(wsdlUrl, content, cancellationToken);
    ...
    public async Task<UpdateSMSProviderResponseModel> Update(SMSProviderUpdateModel model)
    {
        ...
        SMSProvider originalEntity = entity.Clone();
        ...
        {
            string providerId = entity.ProviderName;

            List<SMSProviderConfig> existingConfigs = await _smsProviderConfigRepository
            ...
            if (toDeleteKeys.Count > 0)
            {
                ...
                foreach (SMSProviderConfig del in toDeleteEntities)
                {
                    await _smsProviderConfigRepository.Delete(del);
                }
            }

            IEnumerable<SMSProviderConfigModel> toUpsertRaw = model.SMSProviderConfig.Where(x =>
                !(x.Action?.Equals("delete", StringComparison.OrdinalIgnoreCase) ?? false)
            );
            ...
            foreach (SMSProviderConfig config in toUpsert)
            {
                SMSProviderConfig existing = existingConfigs.FirstOrDefault(x =>
    ...
    {
        bool isExisting = await _smsProviderRepository.Table.AnyAsync(p =>
        ...
        if (model.SMSProviderConfig?.Count > 0)
        {
            ...
            foreach (SMSProviderConfig config in configEntities)
            {
    ...
    {
        List<SMSProviderConfig> headerConfigs = await _smsProviderConfigRepository
            .Table.Where(x =>
        ...
        {
            string headerKey = header.ConfigKey["SOAP_HEADER_".Length..];
