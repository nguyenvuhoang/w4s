...
using O24OpenAPI.Report.API.Application.Utils;
...
public class ViewController(
    IViewerSettingRepository viewerSettingRepository,
    Framework.Services.Logging.ILogger logger,
    ICTHGrpcClientService cthService
) : BasePublicController
{
    StiReport report = EngineContext.Current.Resolve<StiReport>();
    private readonly Framework.Services.Logging.ILogger _logger = logger;
    ...
    public async Task<IActionResult> Reports(string report_code, string token, string lang)
    {
        ...
        var getViewerSetting = await viewerSettingRepository.GetByCodeTemplate(report_code);
    ...
    public async Task<IActionResult> GetReport(string report_code, string token, string lang)
    {
        ...
        try
        {
            var config = await EngineContext
                .Current.Resolve<IReportConfigRepository>()
