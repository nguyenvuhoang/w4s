...
public class GenerateReport(IDataMappingService dataMappingService)
{
    ...
    private readonly IRepository<ReportComponent> reportcomponent = EngineContext.Current.Resolve<IRepository<ReportComponent>>();
    private readonly IRepository<ReportParam> reportparam = EngineContext.Current.Resolve<IRepository<ReportParam>>();
    private readonly IRepository<ReportData> reportdata = EngineContext.Current.Resolve<IRepository<ReportData>>();
    private readonly IRepository<ReportDesign> reportdesign = EngineContext.Current.Resolve<IRepository<ReportDesign>>();
    private readonly IRepository<ReportTemplate> reportTemplate = EngineContext.Current.Resolve<IRepository<ReportTemplate>>();

    public async Task<StiReport> LoadData(ReportConfig reportConfig, StiReport report, UserSessions userSessions, string lang)
    {
        Stopwatch sw = new();
        sw.Start();
        ...
        var reporttemplate = (from rt in reportTemplate.Table
                              where rt.Code == reportConfig.CodeTemplate
                              select rt).FirstOrDefault();

        var reportcomponentlist = await (from rc in reportcomponent.Table
                                         where rc.ReportCode == reportConfig.Code
                                         select rc).ToListAsync();

        var reportparamlist = await (from rs in reportparam.Table
                                     where rs.ReportCode == reportConfig.Code && rs.LangId == lang
                                     orderby rs.Orderby
                                     select rs).ToListAsync();

        var reportdatalist = await (from rs in reportdata.Table
                                    where rs.ReportCode == reportConfig.Code
                                    select rs).ToListAsync();

        var reportdesignlist = await (from rs in reportdesign.Table
                                      where rs.ReportCode == reportConfig.Code
                                      orderby rs.OrderBy
                                      select rs).ToListAsync();
        ...
        bool isViewerInteraction = _report.Dictionary.Variables.ToList().Any(v => !string.IsNullOrEmpty(report.GetValue(v.Name)?.ToString()));
        ...
        else
        {
            ...
            StimulsoftUtils.GenerateDynamicExpressions(_report, reportdesignlist, userInput, _dataMappingService);
            StimulsoftUtils.GenerateDynamicDatabind(_report, reportdatalist, reportdesignlist, userInput, _dataMappingService);
        }
