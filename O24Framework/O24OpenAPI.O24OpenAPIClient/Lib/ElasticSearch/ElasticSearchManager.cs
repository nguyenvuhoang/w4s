...
namespace O24OpenAPI.O24OpenAPIClient.Lib.ElasticSearch;
    ...
    where T : class
{
                        ...
                        m.AutoMap()
                            .Properties(
                                delegate(PropertiesDescriptor<T> ps)
                                {
                                    ...
                                    {
                                        if (
                                            Attribute.IsDefined(property, typeof(TextAttribute))
                                        )
                                        {
                                            ps.Text(
    ...
    private bool __DeleteByQueryContainer(QueryContainer condition)
    {
        ...
        {
            Console.WriteLine(
                "Delete by query failed: " + deleteByQueryResponse.DebugInformation
            );
            return false;
        }
        return true;
    ...
    {
        BytesResponse bytesResponse = _client.LowLevel.Search<BytesResponse>(
            "casa_0",
            jsonQuery
        );
