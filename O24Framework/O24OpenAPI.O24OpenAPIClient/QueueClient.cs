...
using O24OpenAPI.O24OpenAPIClient.Lib.Extensions;
using O24OpenAPI.O24OpenAPIClient.Scheme.Workflow;
using RabbitMQ.Client;
using RabbitMQ.Client.Events;

namespace O24OpenAPI.O24OpenAPIClient;
...
public class QueueClient : MyConsole
{
    ...
    private async Task CommandMessageComingHandler(object sender, BasicDeliverEventArgs e)
    {
        ...
        {
            WFScheme workflow =
                e.Body.ToArray().FromBytes<WFScheme>() ?? throw new InvalidOperationException(
                    "Failed to deserialize JSON to WFScheme."
                );
