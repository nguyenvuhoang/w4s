...
public static class ValidatorExtensions
{
	...
	{
		return ruleBuilder.SetValidator((IPropertyValidator<TModel, decimal>)(object)new DecimalPropertyValidator<TModel, decimal>(maxValue));
	}
	...
	{
		Dictionary<string, JsonElement> dictionary = JsonSerializer.Deserialize<Dictionary<string, JsonElement>>(jsonData);
		...
		foreach (FieldDefinitionModel specification in specifications)
		{
			...
			else
			{
				...
				if (!specification.ValueRange.Nullable && jsonElement.ValueKind == JsonValueKind.Null)
				{
					StringBuilder stringBuilder2 = stringBuilder;
				...
				if (specification.ValueRange.EnumList.Count > 0)
				{
					...
					if (!flag)
					{
						...
						StringBuilder.AppendInterpolatedStringHandler handler = new StringBuilder.AppendInterpolatedStringHandler(48, 2, stringBuilder2);
						handler.AppendLiteral("[");
						handler.AppendFormatted(specification.FieldName);
				...
				if (!string.IsNullOrEmpty(specification.ValueRange.Regex) && jsonElement.ValueKind == JsonValueKind.String && !Regex.IsMatch(jsonElement.GetString(), specification.ValueRange.Regex))
				{
					StringBuilder stringBuilder2 = stringBuilder;
					StringBuilder stringBuilder12 = stringBuilder2;
					StringBuilder.AppendInterpolatedStringHandler handler = new StringBuilder.AppendInterpolatedStringHandler(45, 1, stringBuilder2);
					handler.AppendLiteral("[");
					handler.AppendFormatted(specification.FieldName);
		...
		return string.Join(Environment.NewLine, stringBuilder.ToString().Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries));
	}
}
