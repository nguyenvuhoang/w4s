...
public class APIStarUp : IO24OpenAPIStartup
{
    ...
    public void ConfigureServices(IServiceCollection services, IConfiguration configuration)
    {
        ...
        )
        {
            services.Configure<MvcOptions>(options =>
            {
                ...
                var frameworkXPart = partManager.ApplicationParts.FirstOrDefault(p =>
                    p.Name == "O24OpenAPI.Web.Framework"
                );
        ...
        services.AddCors(options =>
        {
            ...
            {
                var origins = (allowedOriginsSetting ?? string.Empty)
                    .Split(';', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);

                if (origins.Length == 0)
                {
                    Console.WriteLine("⚠️ WARNING: AllowedCorsOrigins not set → USING AllowAnyOrigin()");
