...
) : ITransactionActionService
{
    ...
    private readonly IRepository<TransactionAction> _transactionActionRepository = transactionActionRepository;
        ...
        where T : BaseEntity
    {
        ...
        {
            var pairs = mapping.MasterFields.Split('#', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        ...
        static string ResolveClassShortName(string fullClassName)
        {
            ...
            var parts = fullClassName.Split('.', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
        ...
        Dictionary<string, string> masterFields)
    {
        ...
        var textSearch = string.Join(
            '#',
            searchGL.Select(kv => $"{kv.Key} - {kv.Value}")
        );

        var glDef = await DynamicRepositoryHelper
            .DynamicGetByFields<BaseMasterGL>(masterGLClass, searchGL);
        ...
        int accountingGroup = 1)
    {
        ...
        else
        {
            try
            {
                var query = _glEntriesRepository.Table
           .Where(g => g.TransactionNumber == transactionNumber);
    ...
    public virtual async Task<List<TransactionAction>> ListByCode(string code)
    {
        ...
        return await _transactionActionRepository.Table
            .Where(a => a.TransCode == code)
            .ToListAsync();
    }
         ...
         where T : BaseEntity
    {
        ...
        {
            var pairs = mappingFields.Split('#', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);

            var asyncSetters = new List<Task>();

            foreach (var pair in pairs)
            {
                var items = pair.Split(':', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                if (items.Length == 0)
                {
                    continue;
                }
                ...
                else
                {
                    ...
                    if (valueField.Split('.', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).Length < 2)
                    {
                    ...
                    {
                        asyncSetters.Add(Task.Run(async () =>
                        {
                            var related = await master.GetMasterRelatedStringFieldAsync(valueField).ConfigureAwait(false);
    ...
    /// </summary>
    private static bool TryConvertFromInvariantString(string input, Type targetType, out object? result)
    {
        ...
        try
        {
            ...
            {
                if (int.TryParse(input, NumberStyles.Integer, CultureInfo.InvariantCulture, out var enumInt))
                {
        ...
        where T : BaseEntity
    {
        ...
        var statementType = TypeResolver.RequireType(mapping.StatementClass)
        ...
        if (transaction.IsReverse)
        {
            ...
            {
                await ex.LogErrorAsync($"[ERROR] Reverse update failed for {transaction.TransactionNumber}: {ex.Message}");
            }
        ...
        {
            throw new InvalidOperationException($"Cannot create instance of '{mapping.StatementClass}'.");
        }

        var accountNumber = master.GetMasterStringField(masterValueField);
        if (string.IsNullOrWhiteSpace(accountNumber))
        {
            throw new InvalidOperationException($"Cannot get master value for field '{masterValueField}'.");
        }

        var currencyCode = master.GetMasterStringField(mapping.MasterCurrencyCodeField);
        if (string.IsNullOrWhiteSpace(currencyCode))
        {
            throw new InvalidOperationException($"Cannot get currency code from field '{master + ";" + mapping.MasterCurrencyCodeField}'.");
        }
        ...
        where T : BaseEntity
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException($"Not found config for action '{code}' of {typeof(T).FullName}");
            }

            // Mapping cho master type (không cần gọi lặp lại trong vòng lặp)
            var mapping = await _masterMappingService.GetByMasterClass(typeof(T).FullName!) ?? throw new InvalidOperationException($"Mapping not found for {typeof(T).FullName}");
            string currentAction = string.Empty;

            foreach (var action in actions)
            {
                // Lấy GL rule theo code
                var ruleConfig = await GetGLConfig(master, mapping.GLConfigClass, mapping.MasterGLFields, code);
