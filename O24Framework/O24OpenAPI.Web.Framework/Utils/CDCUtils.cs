...
using O24OpenAPI.Web.Framework.Services.CDC;
...
public class CDCUtils
{
    ...
    private static string FormatValue(object value)
    {
        ...
        switch (value)
        {
            ...
            default:
                var sDefault = Convert.ToString(value, CultureInfo.InvariantCulture) ?? string.Empty;
    ...
    private static string[] GetKeyColumns(string tableName)
    {
        ...
        catch (Exception ex)
        {
            BusinessLogHelper.Error(
                ex,
                $"CDC GetKeyColumns error for table {tableName}"
            );
            return [];
        }
    }
