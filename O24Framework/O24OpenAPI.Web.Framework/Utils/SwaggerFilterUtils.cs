...
public static class SwaggerFilterUtils
{
    ...
    {
        Singleton<ITypeFinder>.Instance.FindClassesOfType<ISwaggerFilter>()
            .ToList()
            .ForEach(type =>
            {
                var filterType = FilterTypeMappings.FirstOrDefault(kv => type.GetCustomAttributes(false).Any(a => a.GetType() == kv.Key)).Value;
                if (filterType != null)
                {
    ...
    private static void RegisterFilter(SwaggerGenOptions options, string filterType, Type type)
    {
        try
        {
            var methodInfo = typeof(SwaggerGenOptionsExtensions).GetMethod(filterType, BindingFlags.Public | BindingFlags.Static);
            if (methodInfo != null)
            {
            ...
            {
                Console.WriteLine($"Failed to find {filterType} method in SwaggerGenOptionsExtensions for {type.FullName}");
            }
        }
        catch (Exception ex)
        {
